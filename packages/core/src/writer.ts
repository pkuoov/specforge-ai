...
import { isCodeExpression } from './generator.js';

function formatInput(value: unknown): string {
  if (isCodeExpression(value)) return value.__testgenExpression;
  ...
  if (typeof value === 'object') {
    ...
    return `{ ${entries.map(([key, entryValue]) => `${formatPropertyKey(key)}: ${formatInput(entryValue)}`).join(', ')} }`;
  }
...
export function renderTestFile(
  ...
  return [
    `// Generated by testgen — https://github.com/pkuoov/specforge-ai`,
